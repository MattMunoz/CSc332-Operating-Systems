...
void transfer(int fds, int fdd){
	...
	while (sz >= 100){
		...
		// checks all charachters in the buffer in a loop and changes every 1 to an A
		for (int i = 0; i < sz; i++) {
			if (c[i] == '1')
				c[i] = 'A';
		}
		
		// writes the buffer content to the destionation file and adds an XYZ after each 
		write(fdd, c, sz);
		write(fdd, "XYZ", 3);
	}
